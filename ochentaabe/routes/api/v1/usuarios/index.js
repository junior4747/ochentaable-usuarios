...
router.put('/update/:codigo', async (req, res) => {
  try {
    ...
    const { email,nombre,avatar,password,estado } = req.body;
    if (/^\s*$/.test(ElementInternals)) {
      return res.status(400).json({
        error: 'Se espera valor de correo'
      });
    ...
    const updateResult = await user.updateCategory({ codigo: parseInt(codigo), email,nombre,avatar,password,estado });
...
router.delete('/delete/:codigo', async (req, res) => {
  try {
    ...
    const deletdusuarios = await user.deleteCategory({ codigo: parseInt(codigo) });

    if (!deletdusuarios) {
      return res.status(404).json({ error: 'Usuario no encontrado.' });
    }
    return res.status(200).json({ deletdusuarios });
